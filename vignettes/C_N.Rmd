...
# get only locations were both OC and N are measured (later we need it like this for cross-correlations)

kenya_afsp2 <- kenya_afsp[!is.na(kenya_afsp$ORCDRC) & !is.na(kenya_afsp$NTO), ]

par(mfrow = c(1,2))
edaplot(kenya_afsp2$ORCDRC[!is.na(kenya_afsp2$ORCDRC)],
        H.freq = TRUE, box = FALSE, S.pch = 3, S.cex = 0.5,
        D.lwd = 1.5, P.ylab = "", P.log = FALSE, P.logfine = c(5,10),
        P.main = "OC", P.xlab = "", B.pch = 3, B.cex = 0.5)
edaplot(log(kenya_afsp2$ORCDRC)[!is.na(kenya_afsp2$ORCDRC)],
        H.freq = TRUE, box = FALSE, S.pch = 3, S.cex = 0.5,
        D.lwd = 1.5, P.ylab = "", P.log = FALSE, P.logfine = c(5,10),
        P.main = "OC logged", P.xlab = "", B.pch = 3, B.cex = 0.5)

spplot(kenya_afsp2[c("ORCDRC", "NTO")])
