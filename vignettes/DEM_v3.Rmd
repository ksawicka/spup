...
loc2 <- 6200  # with an example location of high and low DEM
...
For example, let us identify locations with slopes that are possible (slope > 0.2) and safe (slope < 0.5) for tourist skiing.

```{r, fig.width = 5, fig.height = 3}
# identify areas of slope > 0.2 and < 0.5
slope_q$skiing <- NA
slope_q$skiing[slope_q$prob10perc < 0.5 & slope_q$prob10perc > 0.2] <- "good"
slope_q$skiing[is.na(slope_q$skiing)] <- "bad"
...
spplot(snow_poly, col.regions = c(transparent_grey, "transparent"),  
       main = "Sufficiently certain that suitable for skiing and snow presence",
       sp.layout = spl)
```
