...
author: "Kasia Sawicka and Gerarg Heuvelink"
date: "`r Sys.Date()`"
...
### Adapted Monte Carlo methodology for spatial uncertainty analysis with spatially cross-correlated variables
...
class(OC)
class(TN)
plot(OC, main = "Organic Carbon")
plot(TN, main = "Total Nitrogen")
summary(OC)
summary(TN)
```
...
In order to complete the description of the single uncertain variable we use the `defineUM()` function that collates all information about the DEM uncertainty into one object. The minimum information required is:
...
OCTN_sample <- genSample(UMobject = mySpatialMUM, n = 4, samplemethod = "ugs", nmax = 20, asList = FALSE)
```
...
# or sample from uncertain input and save it in a list by setting asList argument to TRUE (default)
