...
In this simple example, in a neighbourhood the function of the building is assigned depending on the building percentage of area inside the building that is used for living (woonareash) and number of addresses (units) inside each building (vbos). If the residential area is higher than 80% and has at least one address registered then the building is classified as "residential". If the residential area is less than 80% and at least one address is present the building is classified as "office". If no address is present the building function is assumed as "other". Depending of these categories the city council imposes the tax levels of €1000, €10000, €10 per year, accordingly. The 80% threshhold is approximate though and some buildings classified as "others" could have got the address assigned but not put on a system yet, therefore the council wants to calculate the variation in tax revenue depending on building classification. 

<br>

### Adapted Monte Carlo methodology for spatial uncertainty analysis with categorical variable
...
Note that the above ignores uncertainty in model parameters and model structure, but these can easily be included if available as probability distribution functions (pdfs).  
...
The example data for tax calculation contain spatial distribution of buildings in one of the neighbourhoods in Rotterdam, NL. 
...
- Function - assigned category depending on vbos and woonareash - for residential is 1, for offcie is 2, for other is 3,
...
In the nexts section we explain how do we use the information about the probabilities.

<br>

#### Define uncertainty model (UM) for the bulding function

In case of categorical variables the uncertainty is described by use of non-parameteric pdfs. In our case of the neighbourhood we describe it by a vector of probabilities that a building has a certain category. In case of spatially distributed variables we have to do this for each polygon, hence the dataset has maps of these probabilities saved in the same object. 

To collate all the information of the uncertainty model for the building function we use the `defineUM()` function that collates all information into one object. The minimum information required is:

* a logical value that indicates if the object is uncertain,
* a vector of categories; character and numberic type is allowed, 
* a data frame with probabilies for each category.
...
In order to perform uncertainty propagation analysis using 'spup', the model through which uncertainty is propagated needs to be defined as an R function. The 'Rotterdam' data object includes an example of a pre-defined model that calculates slope using building function as input. Note that the model has a requirement that the building function is stored in a SpatialPolygonDataFrame and in a column named 'Function'.
...
We can see that on average the city council should obtain the tax revenue of €2300000, but depending on the building classification this amount may vary by €120000. Basing on this cost estimation the policy makers may adapt more rigorous classification for the tax revenue.
